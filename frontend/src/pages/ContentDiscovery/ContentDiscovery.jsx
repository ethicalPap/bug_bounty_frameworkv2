    ...
    <div className="min-h-screen bg-[#0a0e1a] p-6 space-y-6">
      ...
      <div className="bg-[#151b2e] border border-[#1e2943] rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {['use_ffuf', 'use_feroxbuster'].map(tool => (
            <button
              key={tool}
            ...
            >
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${
                toolConfig[tool] ? 'bg-blue-500 border-blue-500' : 'border-gray-600'
              }`}>
                {toolConfig[tool] && (
              ...
              <span>{tool.replace('use_', '').toUpperCase()}</span>
            </button>
          ))}
          ...
          {['use_waymore', 'use_gau'].map(tool => (
            ...
            >
              ...
              <span className="capitalize">{tool.replace('use_', '')}</span>
            </button>
          ))}
          ...
          {['use_katana', 'use_gospider'].map(tool => (
            <button
              key={tool}
              onClick={() => toggleTool(tool)}
              disabled={scanning}
          ...
          {['use_linkfinder', 'use_arjun', 'use_unfurl', 'use_uro', 'use_nuclei'].map(tool => (
            <button
              key={tool}
              onClick={() => toggleTool(tool)}
              disabled={scanning}
      ...
      {!scanning && results.length > 0 && (
        <>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-yellow-400 flex items-center gap-1 mt-2">
                  <span>💡</span>
                  <span>Export your results above or view detailed analysis in the table below</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            <div className="bg-[#151b2e] border border-[#1e2943] rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-white mb-1">{stats.total}</div>
              <div className="text-xs text-gray-400">Total URLs</div>
            </div>
            <div className="bg-[#151b2e] border border-red-500/20 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-red-400 mb-1">{stats.interesting}</div>
              <div className="text-xs text-gray-400">Interesting</div>
            </div>
            <div className="bg-[#151b2e] border border-purple-500/20 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-purple-400 mb-1">{stats.apis}</div>
              <div className="text-xs text-gray-400">API Endpoints</div>
            </div>
            <div className="bg-[#151b2e] border border-cyan-500/20 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-cyan-400 mb-1">{stats.params}</div>
              <div className="text-xs text-gray-400">Parameters</div>
            </div>
            <div className="bg-[#151b2e] border border-yellow-500/20 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-yellow-400 mb-1">{stats.js_files}</div>
              <div className="text-xs text-gray-400">JS Files</div>
            </div>
            <div className="bg-[#151b2e] border border-green-500/20 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-green-400 mb-1">{stats.status_2xx}</div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-400 mb-4">
                  Export complete list of discovered URLs with status codes, methods, and tool information. 
                  Perfect for further analysis or importing into other security tools.
                </p>
                ...
                <div className="bg-[#0a0e1a] border border-[#1e2943] rounded-lg p-3 mb-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <span className="px-2 py-1 bg-[#151b2e] rounded text-xs text-white">URL</span>
                    <span className="px-2 py-1 bg-[#151b2e] rounded text-xs text-white">Status</span>
                    <span className="px-2 py-1 bg-[#151b2e] rounded text-xs text-white">Method</span>
                    <span className="px-2 py-1 bg-[#151b2e] rounded text-xs text-white">Interesting</span>
                    <span className="px-2 py-1 bg-[#151b2e] rounded text-xs text-white">Tool</span>
                  </div>
                  <div className="text-xs text-gray-500 mt-2">
                    Detailed URL information for security analysis and testing
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-[#151b2e] border border-[#1e2943] rounded-lg overflow-hidden">
            {/* Filters Bar */}
            <div className="bg-[#0a0e1a] border-b border-[#1e2943] p-4">
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  className="flex-1 min-w-[200px] px-3 py-2 bg-[#151b2e] border border-[#1e2943] rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-all"
                />
                <select
                  value={filterType}
                  onChange={(e) => setFilterType(e.target.value)}
                  className="px-3 py-2 bg-[#151b2e] border border-[#1e2943] rounded-lg text-white text-sm focus:outline-none focus:border-blue-500 transition-all"
                >
                ...
                <span className="text-xs text-gray-400 ml-auto">
                  {filteredResults.length} / {results.length} URLs
                </span>
              </div>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-[#0a0e1a] border-b border-[#1e2943]">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Tool</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Type</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Link</th>
                  </tr>
                </thead>
                      ...
                      <tr key={idx} className="hover:bg-[#0a0e1a] transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {item.is_interesting && <AlertCircle size={14} className="text-red-400 flex-shrink-0" />}
                            <span className="text-sm text-white font-mono truncate max-w-md" title={item.discovered_url}>
                              {item.discovered_url}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            ...
                            item.status_code?.toString().startsWith('5') ? 'bg-red-500/20 text-red-400' :
                            'bg-gray-500/20 text-gray-400'
                          }`}>
                            {item.status_code || 'N/A'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-400 uppercase font-medium">
                          {item.method || 'GET'}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            {item.discovered_url?.includes('/api/') && (
                              <span className="px-2 py-0.5 bg-purple-500/20 text-purple-400 rounded text-xs font-medium">API</span>
                            )}
                            {item.discovered_url?.includes('?') && (
                              <span className="px-2 py-0.5 bg-cyan-500/20 text-cyan-400 rounded text-xs font-medium">PARAM</span>
                            )}
                            {item.discovered_url?.endsWith('.js') && (
                              <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-400 rounded text-xs font-medium">JS</span>
                            )}
                            {item.is_interesting && (
                              <span className="px-2 py-0.5 bg-red-500/20 text-red-400 rounded text-xs font-medium">!</span>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
